...
class SessionNotifier extends Notifier<SessionState?> {
  ...
  try {
    await _joinCompleter!.future.timeout(
      ...
      onTimeout: () {
        ...
        throw TimeoutException('Connection timeout. Please check your internet and try again.');
      },
    );
  ...
  void _handleMessage(Map<String, dynamic> message) {
    ...
    if (type == 'error') {
      ...
      // Wait a moment for session_state to arrive with full details
      if (_joinCompleter != null && !_joinCompleter!.isCompleted) {
        if (state == null) {
          debugPrint('✅ FLUTTER - Session update received, waiting for full state...');
          // Don't complete yet, wait for session_state
        } else {
          // We already have state, can complete now
